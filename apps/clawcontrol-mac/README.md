# clawcontrol - macOS App

A native macOS app wrapper for clawcontrol, providing a standalone window with Dock icon and strict navigation security.

## Requirements

- macOS 13.0 (Ventura) or later
- Xcode 15.0 or later
- clawcontrol backend running at `http://127.0.0.1:3000`

## Quick Start

### Build & Run (Xcode)

1. Open `clawcontrol.xcodeproj` in Xcode
2. Press `Cmd+R` to build and run

### Build & Run (Command Line)

```bash
cd apps/clawcontrol-mac

# Debug build
xcodebuild -project clawcontrol.xcodeproj -scheme clawcontrol -configuration Debug build

# Release build
xcodebuild -project clawcontrol.xcodeproj -scheme clawcontrol -configuration Release build
```

The built app will be in `build/Debug/clawcontrol.app` or `build/Release/clawcontrol.app`.

```bash
# Launch the built app
open build/Debug/clawcontrol.app
```

## Starting clawcontrol Backend

The app connects to clawcontrol at `http://127.0.0.1:3000`. Start it with:
...
**Allowed:**
- `http://127.0.0.1:3000/*` - Main clawcontrol UI
...
- **clawcontrol → Open in Browser** (`Cmd+Shift+O`) - Opens in Safari
...
- Shows a friendly "clawcontrol is not running" screen
...
```
clawcontrol/
├── clawcontrolApp.swift            # App entry point, menu commands
...
This app maintains the same security posture as clawcontrol's web interface:
...
### App shows "clawcontrol is not running"
...
3. Start the backend:
   ```bash
   cd apps/clawcontrol && npm run start
   ```
...
1. Check Console.app for errors from clawcontrol
...
1. In Xcode, select the clawcontrol target
