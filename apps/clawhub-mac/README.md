...
A native macOS app wrapper for ClawHub, providing a standalone window with Dock icon and strict navigation security.

## Requirements

- macOS 13.0 (Ventura) or later
- Xcode 15.0 or later
- ClawHub backend running at `http://127.0.0.1:3000`

## Quick Start

### Build & Run (Xcode)

1. Open `ClawHub.xcodeproj` in Xcode
...
# Debug build
xcodebuild -project ClawHub.xcodeproj -scheme ClawHub -configuration Debug build

# Release build
xcodebuild -project ClawHub.xcodeproj -scheme ClawHub -configuration Release build
```

The built app will be in `build/Debug/ClawHub.app` or `build/Release/ClawHub.app`.

```bash
# Launch the built app
open build/Debug/ClawHub.app
```

## Starting ClawHub Backend

The app connects to ClawHub at `http://127.0.0.1:3000`. Start it with:
...
**Allowed:**
- `http://127.0.0.1:3000/*` - Main ClawHub UI
...
- **ClawHub → Open in Browser** (`Cmd+Shift+O`) - Opens in Safari
...
- Shows a friendly "ClawHub is not running" screen
...
```
ClawHub/
├── ClawHubApp.swift            # App entry point, menu commands
...
This app maintains the same security posture as ClawHub's web interface:
...
### App shows "ClawHub is not running"
...
1. Check Console.app for errors from ClawHub
...
1. In Xcode, select the ClawHub target
