...
import type { WorkOrderState, Priority, Owner } from '@clawhub/core'
...
const VIEW_STORAGE_KEY = 'clawhub-work-orders-view'
...
const workOrderColumns: Column<WorkOrderWithOpsDTO>[] = [
  ...
  },
  {
    ...
    render: (row) => (
      <span className={cn(
        'text-xs',
        row.owner === 'agent' ? 'text-status-progress' : 'text-fg-1'
      )}>
        {row.owner === 'agent' ? 'Agent' : 'User'}
      </span>
    ),
  },
  {
    ...
    <div className="space-y-4">
      ...
      <div className="pt-3 border-t border-bd-0 space-y-2">
        <div className="flex justify-between text-xs">
          ...
          <span className="text-fg-1">{workOrder.owner === 'agent' ? 'Agent' : 'User'}</span>
        </div>
        <div className="flex justify-between text-xs">
...
export function WorkOrdersClient() {
  ...
  const filteredWorkOrders = workOrders.filter((wo) => {
    ...
    if (filters.owner !== 'all') {
      const isAgent = wo.owner === 'agent'
      if (filters.owner === 'agent' && !isAgent) return false
