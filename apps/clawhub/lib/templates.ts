...
function ensureMockTemplatesExist() {
  ...
  const sampleTemplateFolder = mockWorkspaceFiles.find(
    (f) => f.path === '/agent-templates' && f.name === 'clawhub-build' && f.type === 'folder'
  )
  ...
  if (!sampleTemplateFolder) {
    ...
    mockWorkspaceFiles.push({
      ...
      name: 'template.json',
      type: 'file',
      path: '/agent-templates/clawhub-build',
      size: 1024,
      modifiedAt: new Date(),
    })
...
## Identity
You are {{agentName}}, a ClawHub build agent working on {{projectName}}.
...
## Notes
Created from clawhub-build template v1.0.0
`
    ...
    mockFileContents['ws_tpl_build_readme'] = `# ClawHub Build Agent Template
...
## Identity
You are {{agentName}}, a ClawHub ${role.toLowerCase()} agent.
