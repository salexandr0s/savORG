 ...
 *
 * Headers:
 * - x-clawhub-actor-id: The actor identifier (e.g., 'claw-alpha', 'admin')
 * - x-clawhub-actor-type: The actor type ('user' | 'system' | 'agent')
 *
...
export function getRequestActor(request: NextRequest): ActorInfo {
  const actorId = request.headers.get('x-clawhub-actor-id')
  const actorType = request.headers.get('x-clawhub-actor-type') as 'user' | 'system' | 'agent' | null
