 ...
 * - ClawHub probes capabilities to detect available features
 * - Graceful degradation when plugin commands are not supported
 * - DB is only used for caching/history, never as canonical truth
 */

import { mockPlugins, mockPluginConfigs } from '@clawhub/core'
import { getDefaultAdapter, runDynamicCommandJson } from '@clawhub/adapters-openclaw'
import {
